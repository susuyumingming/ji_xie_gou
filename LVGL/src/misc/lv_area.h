...
/** Alignments*/
enum {
    LV_ALIGN_DEFAULT = 0,     // 默认对齐（通常为左上角）
    
    // 内部对齐 - 在父容器内对齐
    LV_ALIGN_TOP_LEFT,        // 父容器左上角
    LV_ALIGN_TOP_MID,         // 父容器顶部居中
    LV_ALIGN_TOP_RIGHT,       // 父容器右上角
    LV_ALIGN_BOTTOM_LEFT,     // 父容器左下角
    LV_ALIGN_BOTTOM_MID,      // 父容器底部居中
    LV_ALIGN_BOTTOM_RIGHT,    // 父容器右下角
    LV_ALIGN_LEFT_MID,        // 父容器左侧居中
    LV_ALIGN_RIGHT_MID,       // 父容器右侧居中
    LV_ALIGN_CENTER,          // 父容器正中心

    // 外部对齐 - 相对于其他对象对齐
    LV_ALIGN_OUT_TOP_LEFT,    // 目标对象上方，左对齐
    LV_ALIGN_OUT_TOP_MID,     // 目标对象上方，水平居中
    LV_ALIGN_OUT_TOP_RIGHT,   // 目标对象上方，右对齐
    LV_ALIGN_OUT_BOTTOM_LEFT, // 目标对象下方，左对齐
    LV_ALIGN_OUT_BOTTOM_MID,  // 目标对象下方，水平居中
    LV_ALIGN_OUT_BOTTOM_RIGHT,// 目标对象下方，右对齐
    LV_ALIGN_OUT_LEFT_TOP,    // 目标对象左侧，顶部对齐
    LV_ALIGN_OUT_LEFT_MID,    // 目标对象左侧，垂直居中
    LV_ALIGN_OUT_LEFT_BOTTOM, // 目标对象左侧，底部对齐
    LV_ALIGN_OUT_RIGHT_TOP,   // 目标对象右侧，顶部对齐
    LV_ALIGN_OUT_RIGHT_MID,   // 目标对象右侧，垂直居中
    LV_ALIGN_OUT_RIGHT_BOTTOM,// 目标对象右侧，底部对齐
};
